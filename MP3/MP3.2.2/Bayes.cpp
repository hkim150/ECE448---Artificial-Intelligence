...
void Bayes::train(double laplace_k){
    ...
    for(int i=0; i<image.size(); i++){
        ...
        for(int j=0; j<patches.size(); j++){
            p_of_g_given_class[image[i].getClass()-1][j][patches[j]]++;
        }
    }
...
void Bayes::test(){
    ...
    for(int i=0; i<image.size(); i++){
        ...
        for(int i=0; i<NUM_CLASS; i++){
            ...
            for(int j=0; j<patches.size(); j++){
                aPosteriori += log(p_of_g_given_class[i][j][patches[j]]);
            }
...
void Bayes::evaluate(){
    ...
    for(int i=0; i<image.size(); i++){
        confusionMatrixCount[image[i].getClass()-1][guess[i]]++;
        if(guess[i] == image[i].getClass()-1){correctCount++;}
    }
    cout << "Correctness: " << (double)correctCount/image.size() << endl << endl;
     ...
     for(int i=0; i<NUM_CLASS; i++){
         ...
         for(int j=0; j<NUM_CLASS; j++){
             ...
             cout << j+1 << ": " << confusionMatrix[i][j] << endl;
         }
         cout << endl;
     }
...
bool Bayes::parseImage(string imageFileName, string labelFileName){
    ...
    while(getline(myFile, line)){
        if(!line.empty()){
            currNum = (int)line[0] - 48;
            classCount[currNum-1]++;
            image[count++].setClass(currNum);
        }
    }
    myFile.close();
