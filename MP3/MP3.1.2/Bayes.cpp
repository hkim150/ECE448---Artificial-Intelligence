...
void Bayes::train(double laplace_k){
    ...
    for(int i=0; i<NUM_CLASS; i++){
        p_of_g_given_class[i] = new double* [NUM_PATCH_VAL];
        for(int j=0; j<NUM_PATCH_VAL; j++){
            p_of_g_given_class[i][j] = new double [patchCount];
    ...
    for(int i=0; i<image.size(); i++){
        ...
        for(int j=0; j<patches.size(); j++){
            p_of_g_given_class[image[i].getClass()][patches[j]][j]++;
        }
    }
    ...
    for(int i=0; i<NUM_CLASS; i++){
        for(int j=0; j<NUM_PATCH_VAL; j++){
            for(int k=0; k<patchCount; k++){
                p_of_g_given_class[i][j][k] = p_of_g_given_class[i][j][k] / (classCount[i] + NUM_PATCH_VAL * laplace_k);
            }
        }
    }
}
...
void Bayes::test(){
    ...
    for(int i=0; i<image.size(); i++){
        ...
        for(int i=0; i<NUM_CLASS; i++){
            ...
            for(int j=0; j<patches.size(); j++){
                aPosteriori += log(p_of_g_given_class[i][patches[j]][j]);
            }
            if(num == -1 || aPosteriori > maximumAPosteriori){
